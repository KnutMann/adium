...
{
	/* The containedObjects getter answers with a copy, and under ARC that copy is gone
	 * before this method returns, while the enumeration state the caller keeps points
	 * into it. The price: direct for-in over this object no longer tolerates mutating it
	 * inside the loop; a caller that removes while walking must enumerate a snapshot,
	 * [self containedObjects]. Enumerate the ivar instead, sorted first, exactly as the getter would.
	 */
	if (containedObjectsNeedsSort) {
		containedObjectsNeedsSort = NO;
		[_containedObjects sortUsingFunction:containedContactSort context:(__bridge void *)self];
