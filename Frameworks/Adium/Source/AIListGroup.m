...
{	
	if ([self containsObject:inObject]) {
		/* These arrays may hold the contact's last reference; this local holds on until the
		 * notification below has run, and says until when out loud, because a strong local may
		 * otherwise be given up right after its last mention. */
		NS_VALID_UNTIL_END_OF_SCOPE AIListContact *contact = (AIListContact *)inObject;
		//Remove the object
...
- (void)removeObjectAfterAccountStopsTracking:(AIListObject *)inObject
{
	...
	/* These arrays may hold the object's last reference; this local holds on until the
	 * notification below has run, and says until when out loud, because a strong local may
	 * otherwise be given up right after its last mention. */
	NS_VALID_UNTIL_END_OF_SCOPE AIListObject *removedObject = inObject;
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state objects:(id __unsafe_unretained [])stackbuf count:(NSUInteger)len
{
	 ...
	 * [self containedObjects]. Enumerate the ivar: it lives as long as self, and for-in keeps self alive.
	 */
