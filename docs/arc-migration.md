...
adversarial review pass per cluster. Ten files were skipped for patterns the playbook does not
cover and wait for hand work: AIMessageViewController (self-retain across a sheet),
AISortController (object pointer in a C struct), AIListGroupMockieCell and AIStatusIcons (C arrays
of objects), AIToolbar (dealloc swizzling), AdiumAuthorization (ownership handed across the Purple
boundary), DCJoinChatViewController (self-retain via observer registration),
ESObjectWithProperties (integers pushed through object_setIvar), ESTextAndButtonsWindowController
(two entry points sharing one deferral).
